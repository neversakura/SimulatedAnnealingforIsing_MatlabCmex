...
public:
	Lattice(const mxArray *latticein)
	{
    double *latticeout;
		std::size_t latticeDim;
		latticeDim=mxGetM(latticein);
		latticeout=mxGetPr(latticein);
		maxs = 0;
		links.reserve(32768);
		for (std::size_t i=0; i<latticeDim; ++i)
		{
			int s0, s1;
			value_type cval;
			s0= (int) latticeout[i+latticeDim*0];
			s1= (int) latticeout[i+latticeDim*1];
			cval=(value_type) latticeout[i+latticeDim*2];

			if (s0 < 0 || s1 < 0)
				throw std::runtime_error("negative spin index in lattice config ");
	...
	void init_sites(std::vector<ST>& sites, unsigned maxnb) const
	{
		...
		for (std::size_t i = 0; i < links.size(); ++i) {
			...
			if (link.s0 == link.s1)
				...
				if (nneighb0 >= maxnb)
					throw std::runtime_error("two many neighbors in lattice config; must be less or equal to " + to_s(maxnb));
